...
import hashlib
import requests
from api.sugarcrm import exception
from api.sugarcrm.decorator import valid_parameters
from api.sugarcrm.enumerator import ErrorEnum


class Client(object):
    def __init__(self, url, username, password, app='RestTest', lang='en_US', verify=True, session=None):
    ...
    def _post(self, endpoint, params=None):
        ...
        if self.session:
            ...
            client = requests
        return self._parse(client.post(self.url, data=data, verify=True, timeout=30))
    ...
    def set_entries(self, module_name, name_value_lists):
        ...
        data = [self.session_id, module_name, name_value_lists]
    ...
    def set_relationship(self,module_name,module_id,link_field_name,related_id):
        """
        creating relationship between contacts and accounts 
        """
        print "set_relationship called"
        data = [self.session_id,module_name,module_id,link_field_name,related_id]
        return self._post('set_relationship', data)

    def set_relationships(self,module_names,module_ids,link_field_names,related_ids):
        """
        creating multiple relationship between contacts and accounts 
        """
        print "set_relationships called"
        data = [self.session_id,module_names,module_ids,link_field_names,related_ids]
        print data
        return self._post('set_relationships', data)
