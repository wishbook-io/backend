...
print "catalog"
for puo in Push_User.objects.filter(catalog__isnull=False, id__gt=60000, id__lte=70000).values_list('id', flat=True).order_by('id'):
	print puo
...
print "selection"
for puo in Push_User.objects.filter(selection__isnull=False, id__gt=60000, id__lte=70000).values_list('id', flat=True).order_by('id'):
	print puo
