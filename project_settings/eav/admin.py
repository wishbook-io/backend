...
class ValueAdmin(ModelAdmin):
    list_display = ('entity', 'attribute_name', 'value')
    search_fields = ('entity_id', 'attribute__name')
...
class EnumGroupAdmin(ModelAdmin):
    list_display = ('name', 'many_values')
    search_fields = ('name', 'enums__value')
    filter_horizontal = ('enums', )
    
    def many_values(self, obj):
        value =  obj.enums.all().values_list('value', flat=True)
        return ', '.join(value)
