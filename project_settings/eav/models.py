...
class Attribute(models.Model):
    ...
    def clean(self):
        ...
        if (self.datatype != self.TYPE_ENUM and self.datatype != self.TYPE_MULTI) and self.enum_group:
            raise ValidationError(_(
    ...
    def get_choices(self):
        ...
        '''
        if not (self.datatype == Attribute.TYPE_ENUM or self.datatype == Attribute.TYPE_MULTI):
            return None
    ...
    def save_value(self, entity, value):
        ...
        *value*, the :class:`Value` this attribute for *entity* should
        ...
        if self.datatype == Attribute.TYPE_MULTI:
            value_obj = Value.objects.filter(entity_ct=ct, entity_id=entity.pk, attribute=self, value_enum_id=value.id).first()
            if value_obj is None:
                value_obj = Value.objects.create(entity_ct=ct, entity_id=entity.pk, attribute=self, value_enum_id=value.id)
            return
...
class Value(models.Model):
    ...
    def _get_value(self):
        ...
        if self.attribute.datatype == Attribute.TYPE_MULTI:
            return getattr(self, 'value_%s' % Attribute.TYPE_ENUM)
        else:
    ...
    def _set_value(self, new_value):
        ...
        '''
        if self.attribute.datatype == Attribute.TYPE_MULTI:
            setattr(self, 'value_%s' % Attribute.TYPE_ENUM, new_value)
        else:
